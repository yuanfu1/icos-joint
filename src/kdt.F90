...
!
!  The algorithm is based on Bentley's optimal k-d tree, plus some
!  modifications from the author. The priority queue was omitted since
!  the number of nearest neighbors to be searched is always relatively 
!  small (3-11). 
!
...
SUBROUTINE lls2xyzs(llpts, xyzpts, n)
    ...
    INTEGER :: n
    REAL :: llpts(2, n), xyzpts(3, n)  
   
    INTEGER :: i

    DO i = 1, n
      xyzpts(1,i) = cos(llpts(1,i)) * cos(llpts(2,i))
      xyzpts(2,i) = cos(llpts(1,i)) * sin(llpts(2,i))
      xyzpts(3,i) = sin(llpts(1,i))
    END DO 
